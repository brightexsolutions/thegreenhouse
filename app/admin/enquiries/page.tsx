      ...
      <div className="flex-shrink-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-charcoal/45 mt-0.5">
            ...
            {partnerCount > 0 && <> · <span className="text-[#8a6a1a] font-medium">{partnerCount} partnership</span></>}
          </p>
        </div>
      </div>
        ...
        {enquiries.length === 0 ? (
          ...
          <div className="overflow-y-auto flex-1">
            <table className="w-full">
              <thead className="sticky top-0 bg-white border-b border-mist">
                <tr>
                  ...
                  return (
                    <tr key={e.id} className={`group hover:bg-off-white transition-colors ${isPartner ? "bg-gold/[0.03]" : ""}`}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-charcoal truncate max-w-[160px]">
                              {e.full_name}
                              {isPartner && <span className="ml-1.5 text-[9px] bg-gold/15 text-[#8a6a1a] px-1.5 py-0.5 rounded-full font-semibold uppercase tracking-wide">Partner</span>}
                            </p>
                      ...
                      <td className="px-4 py-4 hidden md:table-cell max-w-[260px]">
                        {e.message ? (
                          <p className="text-xs text-charcoal/55 leading-relaxed line-clamp-2">{e.message}</p>
                        ) : (
